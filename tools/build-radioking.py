...
def build(board, translation, srcdir):
    ...
    os.chdir(path)
    command = "cmake %s -DTRANSLATIONS=%s -DRADIOKING_RELEASE=YES %s" % (cmake_options, translation, srcdir)
    print(command)
    os.system(command)
...
def main():
    parser = argparse.ArgumentParser(description="Build Radioking RC firmware")
